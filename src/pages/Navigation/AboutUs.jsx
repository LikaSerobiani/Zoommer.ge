/* eslint-disable no-unused-vars */
import React from "react";
import { useTranslation } from "react-i18next";

export default function AboutUs() {
  const { t } = useTranslation("global");

  return (
    <div className="my-[30px]">
      <div className="container">
        <div className="border-b-[2px] border-light-grey pb-[30px] mb-[30px]">
          <h1 className="font-bold text-[16px] text-black leading-6">
            {t("footer.aboutUs.title")}
          </h1>
        </div>
        <div className="flex flex-col items-center gap-[30px]">
          {/* heading */}
          <div className="flex flex-col items-center">
            <h1 className="font-bold text-[35px]">
              {" "}
              {t("footer.aboutUs.title")}
            </h1>
            <p className="text-secondary font-bold">
              {t("footer.aboutUs.description1")}
            </p>
          </div>
          {/* description */}
          <div className="flex gap-y-[15px] flex-col w-[70%]">
            <span>{t("footer.aboutUs.description2")}</span>
            <span>{t("footer.aboutUs.description3")}</span>
            <span>{t("footer.aboutUs.description4")}</span>
            <span>{t("footer.aboutUs.description5")}</span>
          </div>
        </div>
      </div>
    </div>
  );
}
